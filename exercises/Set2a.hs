...
isPalindrome str = str == reverse str
    ...
    -- 1 -> True
    -- 2 -> head str == last str
    -- _ -> head str == last str && isPalindrome (drop 1 (init str))
...
palindromify s = if not (isPalindrome s) then palindromify (drop 1 (init s)) else s
...
safeIndex xs i = if i <= length xs - 1 && i >= 0 then Just ((!!) xs i) else Nothing
