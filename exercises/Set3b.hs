...
sums' :: Int -> Int -> [Int]
sums' 1 size = [sums'' size]
sums' i size = sums'' ((size + 1) - i) : sums' (i - 1) size
...
merge (x : xs) (x2 : x2s) = 
    if x <= x2 
        then x   : merge xs (x2 : x2s) 
        else x2  : merge x2s (x : xs)
...
mymaximum bigger initial (x : xs) = mymaximum bigger (num x) xs
    where num xs = if bigger initial xs then initial else xs
