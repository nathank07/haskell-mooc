...
count element foldable = length $ foldr f [] foldable
    where f x acc = if x == element then x : acc else acc
...
inBoth fold1 fold2 = foldr f [] fold1
    where f x acc = if elem x fold2 then x : acc else acc
...
instance Foldable List where
  ...
  foldr fn x Empty = x
  foldr fn x (LNode y z) = fn y (foldr fn x z)
...
instance Foldable TwoList where
  foldr fn x TwoEmpty = x
  foldr fn x (TwoNode y z x1) = fn y (fn z (foldr fn x x1))
