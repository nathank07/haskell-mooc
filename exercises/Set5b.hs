...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node v Empty Empty) = 1
treeSize (Node v left right) = 1 + treeSize left + treeSize right
...
treeMax Empty = 0
treeMax (Node v left right) = max v $ max (treeMax left) (treeMax right)
...
cull val (Node v left right) = if v == val 
                                then Empty 
                                else Node v (cull val left) (cull val right)
...
isOrdered (Node v left right) = allValues (<v) left && allValues (>v) right && isOrdered left && isOrdered right
...
walk xs tree = valAtRoot $ walk' xs tree

walk' :: [Step] -> Tree a -> Tree a
walk' _ Empty = Empty
walk' [] (Node v _ _) = Node v Empty Empty
walk' [StepL] (Node _ Empty _) = Empty
walk' [StepR] (Node _ _ Empty) = Empty
walk' (StepL : xs) (Node v left _) = walk' xs left 
walk' (StepR : xs) (Node v _ right) = walk' xs right
...
set [] val Empty = Empty
set [] val (Node v left right) = Node val left right
set (StepL : xs) val (Node v left right) = Node v (set xs val left) right
set (StepR : xs) val (Node v left right) = Node v left (set xs val right)
...
maybeConcat :: [a] -> Maybe [a] -> [a]
maybeConcat a Nothing = a
maybeConcat a (Just b) = a ++ b

search :: Eq a => a -> Tree a -> Maybe [Step]
search val Empty = Nothing
search val (Node v left right) 
    | val == v = Just []
    | search val left  /= Nothing = Just $ maybeConcat [StepL] $ search val left
    | search val right /= Nothing = Just $ maybeConcat [StepR] $ search val right
    | search val left == Nothing && search val right == Nothing = Nothing
