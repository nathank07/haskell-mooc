...
myGcd a 0 = a
myGcd 0 b = b
myGcd a b = if a > b then myGcd b (a - b) else myGcd b (b - a)
...
smallestDivisor' n attempt = if n `mod` attempt == 0 then attempt else smallestDivisor' n (attempt + 1) 
...
biggestPrimeAtMost n = if not (isPrime n) then biggestPrimeAtMost (n - 1) else n
