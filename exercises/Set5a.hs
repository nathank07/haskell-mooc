...
totalPrice (MkShoppingEntry it price amount) = price * fromIntegral amount 
...
buyOneMore (MkShoppingEntry it price amount) = MkShoppingEntry it price (amount + 1)
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Invert x) = rgbInvert (rgb x)
rgb (Mix x y) = rgbMix (rgb x) (rgb y)

rgbInvert :: [Double] -> [Double]
rgbInvert [x, y, z] = [1 - x, 1 - y, 1 - z]
